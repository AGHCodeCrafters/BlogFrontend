...
const CategoryBox = (props) => {
  ...
  return (
    <ul className="min-h-[50px] p-[10px] mt-[20px] flex items-center justify-between gap-5 overflow-x-auto overflow-y scroll bg-blue rounded-md">
      {categoriesList.map((obj) => (
        <li
          key={Math.floor(Math.random() * 1000)}
          className={`p-[5px] text-white text-16px font-medium border-2 rounded-md ${
